...
resource "aws_organizations_policy" "restrict_ec2_types" {
 ...
 ## Create a Security Policy to Protect Sandbox accounts 
data "aws_iam_policy_document" "ProtectSandbox" {
       Statement {
        Sid = "ProtectSandbox"
        Effect = "Deny"
        Action = [
...
resource "aws_organizations_policy" "ProtectSandbox" {
  name        = "ProtectSandbox"
  description = "Allow certain EC2 instance types only."
  content     = data.aws_iam_policy_document.ProtectSandbox.json
}

 #### To attch policy on Organization Account ########
 resource "aws_organizations_policy_attachment" "ProtectSandbox" {
   policy_id = aws_organizations_policy.ProtectSandbox.id  ## (Required) The unique identifier (ID) of the policy that you want to attach to the target.
  target_id = "ou-gggb-0k7see5q"  ## (Required) The unique identifier (ID) organizational unit, you want to attach the policy to.
 }