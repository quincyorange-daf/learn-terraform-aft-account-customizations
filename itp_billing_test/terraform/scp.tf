...
data "aws_iam_policy_document" "restrict_ec2_type" {
  statement {
...
resource "aws_organizations_policy" "restrict_ec2_type" {
  name        = "restrict_ec2_type"
  description = "Allow certain EC2 instance types only."
  content     = data.aws_iam_policy_document.restrict_ec2_type.json
}

 #### To attch policy on Organization Account ########
 resource "aws_organizations_policy_attachment" "test-scp-custom" {
   policy_id = aws_organizations_policy.restrict_ec2_type.id  ## (Required) The unique identifier (ID) of the policy that you want to attach to the target.
